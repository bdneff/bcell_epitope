#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the VEGF production trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/1BJ1/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/1BJ1/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb
# (VMD topology), md_center.xtc (full-system PBC-clean, unfitted).
# =============================================================================
#SBATCH -J VEGF_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/1BJ1/apo/logs/slurm_post_%j.out
#SBATCH -e md/1BJ1/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/1BJ1/apo/out"
cd "$OUT"
...
echo "=== VEGF apo postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== VEGF postprocess complete $(date) ==="
