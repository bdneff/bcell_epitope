#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the apo Tissue Factor ectodomain (1AHW chain C) trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/1AHW/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/1AHW/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb, md_center.xtc.
# =============================================================================
#SBATCH -J TF_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/1AHW/apo/logs/slurm_post_%j.out
#SBATCH -e md/1AHW/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/1AHW/apo/out"
cd "$OUT"
...
echo "=== apo Tissue Factor ectodomain (1AHW chain C) postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== apo Tissue Factor ectodomain (1AHW chain C) postprocess complete $(date) ==="
