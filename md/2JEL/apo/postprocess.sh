#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the HPr production trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/2JEL/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/2JEL/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb
# (VMD topology), md_center.xtc (full-system PBC-clean, unfitted).
# =============================================================================
#SBATCH -J HPr_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/2JEL/apo/logs/slurm_post_%j.out
#SBATCH -e md/2JEL/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/2JEL/apo/out"
cd "$OUT"
...
echo "=== HPr apo postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== HPr postprocess complete $(date) ==="
