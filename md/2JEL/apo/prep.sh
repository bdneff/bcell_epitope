#!/bin/bash
# =============================================================================
# prep.sh — apo HPr (from 2JEL chain P) system prep. Submit FROM THE REPO ROOT:
#   sbatch md/2JEL/apo/prep.sh
# Input structures/HPr_2JEL.pdb is already antigen-only (chain P, res 1-85, no
# crystal waters/SO4). HPr has no cysteines -> expect 0 disulfides.
# Frozen stack: AMBER99SB-ILDN / TIP3P / cubic 1.2 nm / 0.15 M NaCl.
# Outputs -> md/2JEL/apo/out/ (gitignored); slurm logs -> md/2JEL/apo/logs/.
# =============================================================================
#SBATCH -J HPr_apo_prep
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH -o md/2JEL/apo/logs/slurm_prep_%j.out
#SBATCH -e md/2JEL/apo/logs/slurm_prep_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
RUNDIR="$ROOT/md/2JEL/apo"; CFG="$RUNDIR/configs"; OUT="$RUNDIR/out"
INPUT_PDB="$ROOT/structures/HPr_2JEL.pdb"
[[ -d "$CFG" ]]       || { echo "ERROR: configs not found at $CFG (submit from repo root)"; exit 1; }
[[ -f "$INPUT_PDB" ]] || { echo "ERROR: $INPUT_PDB not found"; exit 1; }
mkdir -p "$OUT"; cd "$OUT"

echo "=== HPr apo prep | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== Disulfide check (HPr: expect 0) ==="; grep -iE "disulfide|S-S|Linking .*CYS" pdb2gmx.log || echo "  (none, as expected)"
...
echo "=== HPr prep complete $(date) — next: sbatch md/2JEL/apo/md.sh ==="
