#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the apo HCMV glycoprotein B protomer (5C6T chain A) trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/5C6T/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/5C6T/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb, md_center.xtc.
# =============================================================================
#SBATCH -J gB_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/5C6T/apo/logs/slurm_post_%j.out
#SBATCH -e md/5C6T/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/5C6T/apo/out"
cd "$OUT"
...
echo "=== apo HCMV glycoprotein B protomer (5C6T chain A) postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== apo HCMV glycoprotein B protomer (5C6T chain A) postprocess complete $(date) ==="
