#!/bin/bash
# =============================================================================
# prep.sh — apo IFN-gamma receptor ectodomain (1JRH chain I) system prep. Submit FROM THE REPO ROOT:
#   sbatch md/1JRH/apo/prep.sh
# Frozen stack: AMBER99SB-ILDN / TIP3P / cubic 1.2 nm / 0.15 M NaCl.
# Outputs -> md/1JRH/apo/out/ (gitignored); slurm logs -> md/1JRH/apo/logs/.
# Expected disulfides: 1.
# =============================================================================
#SBATCH -J IFNgR_apo_prep
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH -o md/1JRH/apo/logs/slurm_prep_%j.out
#SBATCH -e md/1JRH/apo/logs/slurm_prep_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
RUNDIR="$ROOT/md/1JRH/apo"; CFG="$RUNDIR/configs"; OUT="$RUNDIR/out"
INPUT_PDB="$ROOT/structures/IFNgR_1JRH.pdb"
[[ -d "$CFG" ]]       || { echo "ERROR: configs not found at $CFG (submit from repo root)"; exit 1; }
[[ -f "$INPUT_PDB" ]] || { echo "ERROR: $INPUT_PDB not found"; exit 1; }
mkdir -p "$OUT"; cd "$OUT"

echo "=== apo IFN-gamma receptor ectodomain (1JRH chain I) prep | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== Disulfide check (expect 1) ==="; grep -iE "disulfide|S-S|Linking .*CYS" pdb2gmx.log || echo "  (none found)"
...
echo "=== apo IFN-gamma receptor ectodomain (1JRH chain I) prep complete $(date) — next: sbatch md/1JRH/apo/md.sh ==="
