#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the apo IFN-gamma receptor ectodomain (1JRH chain I) trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/1JRH/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/1JRH/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb, md_center.xtc.
# =============================================================================
#SBATCH -J IFNgR_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/1JRH/apo/logs/slurm_post_%j.out
#SBATCH -e md/1JRH/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/1JRH/apo/out"
cd "$OUT"
...
echo "=== apo IFN-gamma receptor ectodomain (1JRH chain I) postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== apo IFN-gamma receptor ectodomain (1JRH chain I) postprocess complete $(date) ==="
