#!/bin/bash
# =============================================================================
# prep.sh — apo Bont/A1 Hc domain (from 2NYY chain A, res 872-1295) prep.
# Submit FROM THE REPO ROOT:  sbatch md/2NYY/apo/prep.sh
# Input structures/BontA1-Hc_2NYY.pdb = C-terminal receptor-binding domain only
# (424 res, no gaps; the catalytic-domain Zn/Ca were excluded with the rest of
# the toxin). NOTE: res 872 is a DOMAIN CUT, so pdb2gmx gives it a standard
# charged N-terminus (artifact of truncation; acceptable for a domain sim).
# 4 Cys (967,1060,1235,1280) -> expect ~2 disulfides.
# Frozen stack: AMBER99SB-ILDN / TIP3P / cubic 1.2 nm / 0.15 M NaCl.
# Outputs -> md/2NYY/apo/out/ (gitignored); slurm logs -> md/2NYY/apo/logs/.
# =============================================================================
#SBATCH -J BontA1Hc_apo_prep
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH -o md/2NYY/apo/logs/slurm_prep_%j.out
#SBATCH -e md/2NYY/apo/logs/slurm_prep_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
RUNDIR="$ROOT/md/2NYY/apo"; CFG="$RUNDIR/configs"; OUT="$RUNDIR/out"
INPUT_PDB="$ROOT/structures/BontA1-Hc_2NYY.pdb"
[[ -d "$CFG" ]]       || { echo "ERROR: configs not found at $CFG (submit from repo root)"; exit 1; }
[[ -f "$INPUT_PDB" ]] || { echo "ERROR: $INPUT_PDB not found"; exit 1; }
mkdir -p "$OUT"; cd "$OUT"

echo "=== Bont/A1 Hc apo prep | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== Disulfide check (Bont/A1 Hc: expect ~2) ==="; grep -iE "disulfide|S-S|Linking .*CYS" pdb2gmx.log || echo "  WARNING: no SS lines matched — inspect pdb2gmx.log"
...
echo "=== Bont/A1 Hc prep complete $(date) — next: sbatch md/2NYY/apo/md.sh ==="
