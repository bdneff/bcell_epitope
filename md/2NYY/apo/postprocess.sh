#!/bin/bash
# =============================================================================
# postprocess.sh — PBC-correct + rot/trans-fit the BontA1Hc production trajectory.
# Submit FROM THE REPO ROOT:  sbatch md/2NYY/apo/postprocess.sh
# Pipeline: -pbc whole -> nojump -> mol/center/compact -> fit rot+trans.
# Outputs (md/2NYY/apo/out/): md_fit.xtc (protein-only, VMD traj), md_ref.pdb
# (VMD topology), md_center.xtc (full-system PBC-clean, unfitted).
# =============================================================================
#SBATCH -J BontA1Hc_apo_post
#SBATCH -p gpu-a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=00:30:00
#SBATCH -o md/2NYY/apo/logs/slurm_post_%j.out
#SBATCH -e md/2NYY/apo/logs/slurm_post_%j.err

set -euo pipefail
ROOT="${SLURM_SUBMIT_DIR:-$PWD}"
OUT="$ROOT/md/2NYY/apo/out"
cd "$OUT"
...
echo "=== BontA1Hc apo postprocess | $(date) | node $(hostname) | job ${SLURM_JOB_ID} ==="
module load Gromacs
...
echo "=== BontA1Hc postprocess complete $(date) ==="
echo "VMD: load md_ref.pdb then md_fit.xtc. Outputs in $OUT (md_fit.xtc, md_ref.pdb, md_center.xtc)."
