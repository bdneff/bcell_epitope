...
actually present at that position in the simulated structure, not assumed from an offset. All six
systems align at zero offset. Two graded-scan antigens are deliberately \emph{not} in this set:
\textbf{MT-SP1} (\texttt{3BN9}/\texttt{3NPS}) has $\ddG$ labels but no apo-MD trajectory yet, and
\textbf{Dengue envelope} (\texttt{1OKE}) has an apo trajectory but Tier-2 \emph{binary} labels
(\S\ref{sec:binary-labels}), which are scored on the binary track rather than joined to the graded
set here. The influenza HA run (\texttt{1HGG}, also Tier-2) is the largest system and had not
finished at the time of this iteration. That leaves $9$ antibody-epitopes over $6$ antigens,
$97$ scanned residues.
...
\texttt{analysis/rmsf\_dynamics\_baseline.py}. MT-SP1 (no apo MD) and Dengue~E (\texttt{1OKE}, Tier-2)
are not in this graded set; HA (\texttt{1HGG}) pending.}\label{fig:rmsf-baseline}
\end{figure}
...
to trust. (iii) MT-SP1 (no apo MD yet) and the Tier-2 systems Dengue~E and HA (\texttt{1HGG} run pending)
are absent from this graded set --- the pooled value will be re-computed as systems join. (iv) As with the static baselines, unscanned residues are treated as non-epitope.
